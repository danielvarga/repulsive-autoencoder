...
class DenseEncoder(Encoder):
    def __init__(self, intermediate_dims, activation):
...
class DenseDecoder(Decoder):
    def __init__(self, latent_dim, intermediate_dims, original_shape, activation):
    ...
    def __call__(self, recons_input):
        ...
        for intermediate_dim in reversed(self.intermediate_dims):
            layers.append(Dense(intermediate_dim))
            layers.append(Activation(self.activation))
        layers.append(Dense(np.prod(self.original_shape), activation='sigmoid', name="decoder_top"))
