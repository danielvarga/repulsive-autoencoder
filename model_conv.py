...
class ConvDecoder(Decoder):
	...
	net = None
        ...
        for i in range(N):
            net = residual_drop_deconv(net, output_shape=(self.batch_size, self.img_size[0], self.img_size[1], filter_num_config[0]), input_shape=(self.batch_size, self.img_size[0], self.img_size[1], filter_num_config[0]))

        conv_f = Convolution2D(filter_num_config[0], 3, 3, border_mode="same", W_regularizer=l2(self.wd))
        decoder_layers.append(conv_f)

        bn_f = BatchNormalization(axis=feat_axis)
        decoder_layers.append(bn_f)

        act_f = Activation("relu")
